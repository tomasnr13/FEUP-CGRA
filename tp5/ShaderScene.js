...
export class ShaderScene extends CGFscene {
	...
	init(application) {
		...
		this.testShaders = [
			...
			new CGFshader(this.gl, "shaders/t6g6.vert", "shaders/t6g6.frag"),
	...
	display() {
		...
		console.log(this.testShaders[this.selectedExampleShader]);
		this.setActiveShader(this.testShaders[this.selectedExampleShader]);
		if(this.selectedExampleShader == 9){
			this.testShaders[this.selectedExampleShader].setUniformsValues({cameracoords:this.camera.position});
		}
		this.pushMatrix();
